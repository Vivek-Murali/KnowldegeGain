...
""" class UserSocailProfile(models.Model):
    user_auth = models.OneToOneField(UserSocialAuth, on_delete=models.CASCADE)
    picture= models.URLField(blank=True, null=True, db_index=True)
    email= models.EmailField(blank=True, null=True, db_index=True)
    email_verified = models.BooleanField(default=False, db_index=True)
    user_id = models.ForeignKey(Profile,related_name="user_id", on_delete=models.CASCADE)
    locale = models.CharField(blank=True, null=True, max_length=50, db_index=True)
    
    @property
    def image_url(self) -> str:
        return requests.get("https://www.googleapis.com/oauth2/v3/userinfo?access_token={}".format(json.loads(self.user_auth.extra_data)['access_token']))['picture']
